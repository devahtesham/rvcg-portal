...
import { DeletePropertyFeature, DeletePropertyTypes, GetPropertyFeatures, GetPropertyTypes } from '../../../store/slices/propertyManagementSlice/propertyManagementSlice';
import MapLoader from '../../Loader/MapLoader';
import { errorNotify } from '../../../Toastify/Toastify';
import AddPropertyFeature from './AddPropertyFeature';

const PropertyFeatures = () => {
  const [isEdit, setIsEdit] = useState(false)
  const [document, setDocument] = useState(false)
  const dispatch = useDispatch();
  const { isLoading, propertyFeatures } = useSelector((state) => state.PropertyMangementReducer)
  ...
  const deleteHandler = (value) => {
    ...
    dispatch(DeletePropertyFeature(value.id))
      .then((response) => {
  ...
  return (
    <>
            ...
            <div className="property-listing-grid mt-5">
              <Row className='justify-content-center'>
                <Col lg={11}>
                  ...
                  <DataGridGlobal
                    data={propertyFeatures.length ? propertyFeatures : []}
                    columns={columns}
                    options={options}
                  />
                </Col>
      ...
      <ModalGlobal>
        <AddPropertyFeature isEdit={isEdit} data={document} />
      </ModalGlobal>
