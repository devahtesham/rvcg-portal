...
import { DeletePropertyTypes, GetPropertyTypes } from '../../../store/slices/propertyManagementSlice/propertyManagementSlice';
...
const PropertyType = () => {
  ...
  const { isLoading, propertyTypes } = useSelector((state) => state.PropertyMangementReducer)
  useEffect(() => {
  ...
  const deleteHandler = (value) => {
    ...
    dispatch(DeletePropertyTypes(value.id))
      .then((response) => {
  ...
  return (
    <>
            ...
            <div className="property-listing-grid mt-5">
              <Row className='justify-content-center'>
                <Col lg={11}>
                  ...
                  <DataGridGlobal
                    data={propertyTypes.length ? propertyTypes : []}
                    columns={columns}
                    options={options}
                  />
                </Col>
      ...
      <ModalGlobal>
        <AddPropertyType isEdit={isEdit} data={document} />
      </ModalGlobal>
